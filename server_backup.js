...
app.put('/executethis', function(req, res) {
    ...
    if(reservedParameters.has("executeThis"))
    {
...
function loopWidRequests(rot,fullURL,mulWidResKeys,mulWidVal,valRetCol,_resu,cb)
{
    ...
    request({
      ...
      uri: fullURL,
      headers:{ 'Content-type' : 'application/json'},
      body: JSON.stringify(convertToParamsInDTOFormat(paramsFromDB))
    }, function (error, response, body) {
      if(response.statusCode == 200){
          console.log("Success --> "+body);
        loopWidRequests(--rot,fullURL,mulWidResKeys,mulWidVal,valRetCol,JSON.stringify(body),cb);
      } else {
        loopWidRequests(--rot,fullURL,mulWidResKeys,mulWidVal,valRetCol,response.statusCode,cb);
      }
    });   
    }else
    {
        loopWidRequests(--rot,fullURL,mulWidResKeys,mulWidVal,valRetCol,JSON.stringify(""),cb);
    }
}
/*
