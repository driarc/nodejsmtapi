...
// 	// cleanparameters changes resultobject based on the diff dtotype and method
// 	// the change could be an add (i.e. add preamble to paramters) or remove (remove preamble)

// 	// LM: This 'if' was not catching undefined so it was not adopting the metadata.method
...
function aggressivedto(widName, parameterObject) { // returns a made up dto base on maximum number of relationships, etc
	var moreDTOParameters={};
	var dtoGlobalParameters={};
	moreDTOParameters = simpleQuery(widName, "attributes", "all", "forward", "", "", "");
...
/*	// ----------------
	...
	// --------------------*/
	if (dtotype=="") {dtotype=resultObj["metadata.method"]}; // added 10-8 late
	if (cleanmethod=="remove") {
...
*/
    	...
    	inputParametersObject = addclean(inputParametersObject, dtotype, accesstoken, "add", convertmethod);
    	//inputParametersObject=inputParametersObject.parms; // ************ prob dont need this
    	//inputParametersObject = inputParametersObject.dto;   // ************
    	metadata=inputParametersObject['metadata.method'];
